...
@section('content')
    <main class="max-w-3xl mx-auto px-6 py-20">
        ...
        : 'https://placehold.co/200x200' }}" alt="Event" class="w-24 h-24 rounded-2xl object-cover">
                    <div>
                        ...
                        <p class="text-slate-500">{{ \Carbon\Carbon::parse($event->date)->format('d M Y') }} •
                            {{ $event->location }}</p>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                <h3 class="text-xl font-bold mb-6 italic text-indigo-600 underline underline-offset-8">📦 Data Pemesan
                    (Tanpa Login)</h3>
                <form class="space-y-6" id="checkout-form" action="{{ route('checkout.store', $event->id) }}" method="POST">
                    <div>
                    ...
                    <button type="button" onclick="showMidtrans()"
                        class="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-xl shadow-xl shadow-indigo-200 hover:bg-indigo-700">
                        Lanjut Pembayaran
                    </button>
    ...
    <div id="midtrans-overlay"
        ...
        <div class="bg-white w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl animate-bounce-in">
            ...
            <div class="p-8 text-center">
                ...
                <h2 class="text-3xl font-black text-indigo-700 my-2">Rp
                    {{ number_format($event->price + 5000, 0, ',', '.') }}</h2>
                <p class="text-xs text-slate-400">Order ID #TRX-DUMMY</p>
    ...
    <script>
        function showMidtrans() {
            // Ubah display flex agar sesuai dengan tailwind
        ...
        function goToTiket() {
            ...
            window.location.href = '{{ route("ticket", $event->id) }}?order_id=' +orderId;
        }
    </script>
