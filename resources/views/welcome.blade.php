...
@section('content')
    <section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
        ...
        <div class="flex-1 relative">
            ...
            @if($events->count() > 0)
                <img src="{{ asset('storage/' . trim($events->first()->poster_path)) }}" alt="{{ $events->first()->title }}"
                    class="rounded-[2rem] shadow-2xl relative z-10 w-full object-cover aspect-[4/5] object-center">
            @else
                <div class="rounded-[2rem] shadow-2xl relative z-10 w-full aspect-[4/5] bg-slate-100 flex items-center justify-center text-slate-400 font-medium border border-slate-200">
    ...
    <section class="max-w-7xl mx-auto px-6 py-10 border-t border-b border-slate-100 my-10">
        <p class="text-center text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Didukung Oleh Partner Resmi</p>
        <div class="flex flex-wrap items-center justify-center gap-12 opacity-70">
            @forelse($partners as $partner)
                <div class="flex flex-col items-center gap-2 group">
                    <img src="{{ asset('storage/' . trim($partner->logo_url)) }}" alt="{{ $partner->name }}" class="h-10 max-w-[140px] object-contain group-hover:grayscale-0 transition duration-300">
                    <span class="text-[11px] font-bold text-slate-400 tracking-wider uppercase opacity-0 group-hover:opacity-100 transition duration-300">{{ $partner->name }}</span>
                </div>
            @empty
                <p class="text-xs text-slate-400 w-full text-center italic">Belum ada partner resmi terdaftar.</p>
            @endforelse
        </div>
    </section>
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-12 gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <button class="px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold shadow-md shadow-indigo-100">Semua</button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($events as $event)
                <div class="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden">
                    <div class="relative overflow-hidden aspect-[3/4]">
                        <img src="{{ asset('storage/' . trim($event->poster_path)) }}" alt="{{ $event->title }}"
            ...
            @empty
                <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-16 bg-slate-50 rounded-[2rem] border border-dashed border-slate-200">
                    <p class="text-slate-400 font-medium">Belum ada data event terdaftar saat ini.</p>
                </div>
            @endforelse
        </div>
    </section>
